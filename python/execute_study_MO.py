...
os.environ['CUDA_VISIBLE_DEVICES'] = "-1" # -1: run on cpu, 0: run on gpu (first)
...
def generate_model(network_param_dict, verbosity=1):
    ...
    # FLOPS calc profiling
    flops = get_flops(model) #, batch_size=1)
    #flops = (flops_raw / 10 ** 9) #convert to gigaflops
    #print(f"FLOPS: {flops_raw / 10 ** 9:.03} G")
    print(f"FLOPS: {flops}")
...
def objective_fn(trial, X_normalized, y_normalized, sampler_choice):
    ...
    model, flops = generate_model(network_params)
    ...
    print(f"FLOPS for trial {trial.number}: {flops}")  # Debug-Ausgabe
    ...
    # GE_range = 0.01
    # params_min = 168
    # params_range = 10080 #15*168
    # combined_metric = (GE - GE_min) / GE_range + alpha * (learnable_params - params_min) / params_range
...
def learn(network_params, optimizer_params, X_normalized, y_normalized, batch_size, trial):
    ...
    for train_out_ix, test_ix in cv_outer.split(X_normalized):
        ...
        Networkmodel_hcci, flops = generate_model(network_params, verbosity)
        ...
        log_dir_in = f'./experiment_logs/model_dir_BO_Learn/val_{fold_out}_{time_stamp}_trial{trial.number}'
        model_dir_in = f"./experiment_logs/log_dir_BO_Learn/val_{fold_out}_{time_stamp}_trial{trial.number}.weights.h5"
        ...
        model_dir_out = f'./experiment_logs/model_dir_BO_Learn/test_{fold_out}_{time_stamp}_trial{trial.number}.weights.h5'
        log_dir_out = f"./experiment_logs/log_dir_BO_Learn/test_{fold_out}_{time_stamp}_trial{trial.number}"
        
        early_stop = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=20, start_from_epoch=epochs_start, verbose=1)
        checkpoint = tf.keras.callbacks.ModelCheckpoint(model_dir_out, monitor="val_loss", mode="min", save_best_only=True, save_weights_only=True, verbose=0)
...
def search_BO(n_trials=trials, filename=filename):
    best_dict = {}
    ...
    #best_trial = study.best_trial #best_trial if one direction only!    

    print("Pareto front:")
    best_trials = sorted(study.best_trials, key=lambda t: t.values)
    for trial in best_trials:
        print("  Trial#{}".format(trial.number))
        print("    Values: GE={}, Learnables={}".format(trial.values[0], trial.values[1])) #FLOPS
...
best_dict = search_BO(n_trials=trials, filename=filename)
